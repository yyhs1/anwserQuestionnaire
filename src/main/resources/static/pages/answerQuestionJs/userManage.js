                ...
                if (res.code == "666") {
                    ...
                    if (userInfo.length) {
                        for (var i = 0; i < userInfo.length; i++) {
                            ...
                            dataNewObj.stopTime = userInfo[i].stopTime;
...
function resetPassword(id) {
    commonAjaxPost(true, "/admin/modifyUserInfo", {'id': id, 'password': '123'}, function (data) {
        if (data.code === "666") {
            getUserList();
            alert(data.message + ",新密码为123");
        } else {
            alert(data.message);
        }
    });
...
function changeStatus(index) {
    alert("修改用户状态")
    commonAjaxPost(true, "/admin/modifyUserInfoStatus", {'id': index, 'status': 0}, function (data) {
        if (data.code === "666") {
            getUserList();
        }
        alert(data.message);
...
function deleteUser(id) {
    alert("删除用户");
    commonAjaxPost(true, "/admin/deleteUserInfoById", {'id': id}, function (data) {
        if (data.code === "666") {
            getUserList();
        }
        alert(data.message);
